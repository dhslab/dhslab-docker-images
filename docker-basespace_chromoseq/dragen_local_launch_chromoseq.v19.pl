...
BEGIN {
	...
	my $obj = { _inputs => '/gscmnt/gc2555/spencer/dhs/git/docker-basespace_chromoseq/inputs.v19.local.json',
		    _wdl => '/gscmnt/gc2555/spencer/dhs/git/docker-basespace_chromoseq/Chromoseq.v19.wdl',
	...
	foreach my $i (0..$#{$obj->{_samples}{$s}}){	    
	    ...
	    my $s = join(',',$dat{read_group_header}{ID},$dat{read_group_header}{LB},$dat{read_group_header}{LB},$dat{index_illumina}{lane},$R1,$R2);
	    `echo $s >> $fastqfile`;
	...
	foreach my $samplename (keys %samples){
	    my $sample = $samplename; #{$samplename};
...
my $usage = "$0 <command> [options]\n\nCommands:\n\tflowcells -r|run <run name> -o|out <output directory> <flowcell directory1> [flowcell directory 2...]\n\tfastqs -r|run <run name> -o|out <output directory> <fastq directory 1> [fastq directory2 ...]\n\talignments [-g|gender male/female] -o|out <output directory> <dragen output directory 1> [dragen output directory2 ...]\n";
